...
$tbl .='<br><div style="font-size:14px;color:grey;">';
$tbl .= str_replace(",", " | ", $this->session->userdata('cadd') ). ' <br /> ' . "Phone :" . $this->session->userdata('contact') . " | " . $this->session->userdata('email') . " | " . $this->session->userdata('website') .  '<br />
<strong style="font-size:20px;padding-top:10px;color:red;">GSTIN : ' . $this->session->userdata('gstin') . '</div></strong></td>
</tr>';
...
//<button type="button" class="btn btn-primary btn-circle btn-xs center-block" href="#" onclick="pdfTransid('. $d['id']. ')"><i class="fa fa-pdf"></i></button>;
...
<button type='button' class='btn btn-danger btn-circle btn-xs center-block' href='#' onclick='pdfTransid(". $d["id"]. ")'><i style='color:white;' class='fa fa fa-file-pdf'></i></button>
*/
$button="<div class='btn-group'><button type='button' class='btn btn-warning btn-circle btn-xs center-block'  href='#' data-toggle='modal' data-target='#modalEditSales'  onclick='updateTransid(" . $d["id"] . ")'><i class='fa fa-edit'></i></button><button type='button' class='btn btn-primary btn-circle btn-xs center-block' href='#' onclick='printTransid(". $d["id"]. ")'><i class='fa fa-print'></i></button></div>";
...
*/
$button="<div class='btn-group'><button type='button' class='btn btn-info btn-circle btn-xs center-block'  href='#' data-toggle='modal' data-target='#modalEditSales'  onclick='updateTransid(" . $d["id"] . ")'><i class='fa fa-edit'></i></button>&nbsp;<button type='button' class='btn btn-danger btn-circle btn-xs center-block'  href='#' data-toggle='modal' data-target='#deleteModal'  onclick='delTransid(" . $d["id"] . ")'><i class='fa fa-times'></i></button></div>";
...
if($settArr)
{
    # code...
  //var_dump($settArr);
    $iprefix = $settArr['inv_prefix'];
    $isuffix = $settArr['inv_suffix'];
    $inv_no = $settArr['inv_no'];
    $inv_numtype = $settArr['inv_numtype'];
    $inv_leadingzero = $settArr['leading_zero'];
...
{
    $id = $this->input->get('id');
    $del_reason = $this->input->get('dreason');
//$id = $this->input->post('delreason');
//var_dump($id);
...
$url=$this->config->item("api_url") . "/api/transaction/trans_byid/" . $id;
//$data = array("id"=>$id,"finyear"=>$finyear,"compId"=>$compId,"trans_type"=>$trans_type);
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL,$url);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  ...
  $transbyidresponse = curl_exec($ch);
//var_dump($salesbyidresponse);
curl_close($ch); // Close the connection
$obj_trans = json_decode($transbyidresponse,true);

//var_dump($obj_trans);
//var_dump($transbyidresponse[0].trans_id);

foreach ($obj_trans as $key => $otvalue) {
    // code...
  //  var_dump($otvalue);
$trans_id = $otvalue['trans_id'];
$trans_date = $otvalue['trans_date'];
$trans_type = $otvalue['trans_type'];
$cust_name = $otvalue['custname'];
$net_amount = $otvalue['net_amount'];

}


//Delete req
$url=$this->config->item("api_url") . "/api/transaction/delpursal_byid/" . $id . "/" . $finyear . "/" . $compId ;
$data_array = array("delflag"=>1);
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL,$url);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  ...
  $delpurbyidresponse = curl_exec($ch);
//var_dump($delpurbyidresponse);
curl_close($ch); // Close the connection
$obj = json_decode($delpurbyidresponse,true);
//var_dump($obj);
if($obj['success']==true)
{
    //var_dump($obj['success']);

//Post log data to audit log table    
$url=$this->config->item("api_url") . "/api/transaction/post_auditlog";
            $log_txt="Deleted record Id:" . $id . "  on Purchase Tbl with the reason " . $del_reason . " deleted by user : " . $login;
            $logdata=array("log_description"=>$log_txt,"log_date"=>date("Y-m-d"));
$log_txt="Deleted record id #" . $id . " Bill No. " . $trans_id . " dated : " . $trans_date . " Net Amount : " . $net_amount . " deleted by user : " . $login;
$data_array=array(
    
    "log_date"=>date("Y-m-d"),
    "log_description"=>$log_txt,
    "company_id"=>$compId);
...
//$url=$this->config->item("api_url") . "/api/insert_transaction.php";
  ...
  curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");    
  curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($data_array));
  $log_insresponse = curl_exec($ch);
 // var_dump($response);
  //echo $response;
  //$result = json_decode($response);
  curl_close($ch); // Close the connection

$res = json_decode($log_insresponse,true);

}
//echo $log_insresponse;
}
